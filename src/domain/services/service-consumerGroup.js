...
exports.Create = async (req, res) => {
  let status = 'Success',
    ...
    response = {};
  try {
    const { name, monthOrders, consumers, anualOrders} = req.body
    if (name) {
      let respOrm = await ormConsumerGroup.Create(
        name,
        consumers,
        monthOrders,
        anualOrders,
...
exports.Update = async (req, res) => {
  ...
  try {
    const { id } = req.params;
    const { name, anualOrders, monthOrders, consumers } = req.body;
    ...
    if (id && updatedConsumerGroup) {
      ...
      console.log('respOrm', respOrm, updatedConsumerGroup);
      if (respOrm?.err) {
        (status = 'Failure'),
          (errorcode = respOrm.err.code),
          (message = respOrm.err.messsage),
